package com.dreamsoftware.coins4all.presentation.detail.viewModel

import androidx.compose.material3.ExperimentalMaterial3Api
import androidx.compose.material3.TopAppBarDefaults
import androidx.compose.material3.TopAppBarState
import androidx.compose.runtime.State
import androidx.compose.runtime.mutableStateOf
import androidx.lifecycle.SavedStateHandle
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.flow.launchIn
import kotlinx.coroutines.flow.onEach
import kotlinx.coroutines.launch
import com.dreamsoftware.coins4all.core.constants.Constants.COIN_ID_PARAM
import com.dreamsoftware.coins4all.domain.model.CoinCurrency
import com.dreamsoftware.coins4all.domain.model.CoinDetail
import com.dreamsoftware.coins4all.domain.usecase.AddFavoriteCoinUseCase
import com.dreamsoftware.coins4all.domain.usecase.CheckFavoriteCoinUseCase
import com.dreamsoftware.coins4all.domain.usecase.RemoveFavoriteCoinUseCase
import com.dreamsoftware.coins4all.domain.usecase.GetCoinUseCase
import com.dreamsoftware.coins4all.domain.usecase.GetHistoricalCurrencyUseCase
import com.dreamsoftware.coins4all.presentation.ui.utils.Action
import com.dreamsoftware.coins4all.presentation.ui.utils.UIState
import java.text.SimpleDateFormat
import java.util.*
import javax.inject.Inject

@HiltViewModel
class CoinDetailsViewModel @Inject constructor(
    private val getCoinUseCase: GetCoinUseCase,
    private val getHistoricalCurrencyUseCase: GetHistoricalCurrencyUseCase,
    private val checkFavoriteCoinUseCase: CheckFavoriteCoinUseCase,
    private val addFavoriteCoinUseCase: AddFavoriteCoinUseCase,
    private val removeFavoriteCoinUseCase: RemoveFavoriteCoinUseCase,
    savedStateHandle: SavedStateHandle
) : ViewModel() {

    @ExperimentalMaterial3Api
    val scrollBehavior = mutableStateOf(TopAppBarDefaults.enterAlwaysScrollBehavior(state = TopAppBarState(0.0F,0.0F,0.0F)))

    private val _coinDetailState = mutableStateOf<UIState<CoinDetail?>>(UIState.Empty())
    val coinDetailState: State<UIState<CoinDetail?>> = _coinDetailState

    private val _currencyHistoryState = mutableStateOf<UIState<List<CoinCurrency>>>(UIState.Empty())
    val currencyHistoryState: State<UIState<List<CoinCurrency>>> = _currencyHistoryState

    private val _isFavorite = mutableStateOf(false)
    val isFavorite: State<Boolean> = _isFavorite

    private var id = ""


    init {
        savedStateHandle.get<String>(COIN_ID_PARAM)?.let {
            id = it
            reload(id)

            viewModelScope.launch {
                _isFavorite.value = checkFavoriteCoinUseCase.invoke(id)
            }
        }
    }

    fun reload(it: String = id) {
        getCoin(it)
        val timestamp = System.currentTimeMillis()
        val month = 24L * 60 * 60 * 31 * 1000 * 11
        val end = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(
            timestamp
        )
        val start = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(
            timestamp - month
        )
        getHistoricalCurrency(it, start, end)
    }

    fun processFavorites(id: String) {
        viewModelScope.launch {
            if (_isFavorite.value) removeFavoriteCoinUseCase(id)
            else addFavoriteCoinUseCase(id)

            _isFavorite.value = !_isFavorite.value
        }
    }

    private fun getCoin(id: String) {
        getCoinUseCase(id).onEach { result ->
            _coinDetailState.value = when (result) {
                is Action.Success -> {
                    UIState.Success(result.data)
                }
                is Action.Empty -> {
                    UIState.Empty(result.message)
                }
                is Action.Loading -> {
                    UIState.Loading()
                }
            }
        }.launchIn(viewModelScope)
    }

    private fun getHistoricalCurrency(id: String, start: String, end: String) {
        getHistoricalCurrencyUseCase(id, start, end).onEach { result ->
            _currencyHistoryState.value = when (result) {
                is Action.Success -> {
                     UIState.Success(result.data ?: emptyList())
                }
                is Action.Empty -> {
                   UIState.Empty(result.message)
                }
                is Action.Loading -> {
                    UIState.Loading()
                }
            }
        }.launchIn(viewModelScope)
    }

}